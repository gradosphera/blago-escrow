...
export function blagoEscrowConfigToCell(config: BlagoEscrowConfig): Cell {
    return (
        beginCell()
            ...
            // eslint-disable-next-line no-null/no-null
            .storeAddress(null) // blago_jetton_wallet (initially null)
            .storeDict(Dictionary.empty(Dictionary.Keys.Uint(32), Dictionary.Values.Cell()))
            .endCell()
    );
}

export class BlagoEscrow implements Contract {
    constructor(
    ...
    static createFromAddress(address: Address) {
        return new BlagoEscrow(address);
    }

    static createFromConfig(config: BlagoEscrowConfig, code: Cell, workchain = 0) {
        const data = blagoEscrowConfigToCell(config);
        const init = { code, data };
        return new BlagoEscrow(contractAddress(workchain, init), init);
    }
    ...
    async sendCreateCheck(
        ...
        const body = BlagoEscrow.prepareCreateCheck(opts);
    ...
    static prepareCreateJettonCheck(
        ...
        return beginCell()
            ...
            .storeRef(BlagoEscrow.prepareCreateJettonCheckForwardPayload(opts))
            .endCell();
    }

    static prepareCreateJettonCheckForwardPayload(opts: {
        checkId: number;
        chatInstance?: string;
        username?: string;
        comment?: string;
    }) {
        return BlagoEscrow.prepareCreateCheck(opts);
    }
